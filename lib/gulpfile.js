...
const transformSass = require('lutool-build/lib/transformSass');
...
function dist(done) {
  ...
  runCmd('lutool-build', ['--devtool=#sourcemap'], (code) => {
    done(code);
  });
}
    ...
    .pipe(through2.obj(function (file, encoding, next) {
      ...
      if (file.path.match(/\/style\/index(\.web)?\.js/)) {
        ...
        if (file.path.indexOf('.web.js') === -1 &&
          (content.indexOf('.scss\'') === -1 || content.indexOf('\'react-native\'') !== -1)) {
          next();
          return;
        }
...
gulp.task('compile', () => {
  execSync('rm -rf lib');
  const sass = gulp.src(['components/**/*.scss'])
    .pipe(through2.obj(function (file, encoding, next) {
      this.push(file.clone());
      if (file.path.match(/\/style\/index(\.web)?\.scss$/)) {
        transformSass(file.path).then((css) => {
...
function publish(tagString, done) {
  let args = ['publish', '--with-lud-tools'];
  if (tagString) {
...
gulp.task('guard', (done) => {
  ...
  if (npmArgs) {
    for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
      if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-lud-tools') < 0) {
        reportError();
        done(1);
        return;
      }
